...
def _result_to_frames(result: dict) -> tuple:
    ...
    return (
        dash_df, dd_df, up_df, mapped_df, crosswalk_df, sharepoint_df,
        dataset_dataflow_df, e2e_df, result["summary"],
    )
...
def _load_from_excel(path: Path) -> tuple:
    ...
    return (
        dash_df, dd_df, up_df, mapped_df, crosswalk_df, sharepoint_df,
        dataset_dataflow_df, e2e_df, summary,
    )
...
if st.button(
    ...
    (
        dash_df, dd_df, up_df, mapped_df, crosswalk_df, sharepoint_df,
        dataset_dataflow_df, e2e_df, summary,
    ) = _result_to_frames(result)
    st.session_state["linhagem_result"] = {
        "dash_df": dash_df, "dd_df": dd_df, "up_df": up_df, "mapped_df": mapped_df,
        "crosswalk_df": crosswalk_df, "sharepoint_df": sharepoint_df,
        "dataset_dataflow_df": dataset_dataflow_df, "e2e_df": e2e_df, "summary": summary,
