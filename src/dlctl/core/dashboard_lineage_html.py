...
<section class="panel"><div class="panel-head"><h2>Itens do mapping</h2><span class="note">Clique numa linha para inspecionar</span></div><div class="table-wrap"><table><thead><tr><th>Tabela / entrada</th><th>Camada</th><th>Domínio</th><th>Status</th><th>Etapa</th><th>Dashboards</th></tr></thead><tbody id="rows"></tbody></table></div></section>
...
const statuses=[...new Set(rows.map(r=>r.status_fim_a_fim))].sort(), layers=[...new Set(rows.map(r=>r.camada_mapeada).filter(Boolean))].sort();
...
function apply(){
 const q=$('search').value.trim().toLocaleLowerCase('pt-BR'), st=$('status').value, la=$('layer').value, re=$('reaches').value;
 filtered=rows.filter(r=>(!st||r.status_fim_a_fim===st)&&(!la||r.camada_mapeada===la)&&(!re||r.chega_dashboard===re)&&(!q||Object.values(r).join(' ').toLocaleLowerCase('pt-BR').includes(q)));
 $('visibleCount').innerHTML=`<span class="count">${fmt(filtered.length)}</span> de ${fmt(rows.length)} itens`;
 $('rows').innerHTML=filtered.map((r,i)=>`<tr data-i="${i}"><td><strong>${esc(r.tabela_mapeada)}</strong></td><td>${esc(r.camada_mapeada)}</td><td>${esc(r.dominio)}</td><td><span class="status ${statusClass(r.status_fim_a_fim)}">${esc(statusLabel(r.status_fim_a_fim))}</span></td><td>${esc(r.etapa_alcancada)}</td><td>${fmt(r.qtd_dashboards)}</td></tr>`).join('')||'<tr><td colspan="6" class="empty">Nenhum resultado.</td></tr>';
...
function select(r,tr){
 ...
 stages.push({c:'origin',n:'1. Mapping / origem',v:path[0]||r.tabela_mapeada,s:r.dominio||'Sem domínio'});
...
['search','status','layer','reaches'].forEach(id=>$(id).addEventListener(id==='search'?'input':'change',apply));
$('clear').onclick=()=>{$('search').value='';$('status').value='';$('layer').value='';$('reaches').value='';apply()};
