...
<body><main class="app">
<header class="header"><div><div class="eyebrow" id="eyebrow">Constellation · Auditoria estática</div><h1>Mapa fim a fim de linhagem</h1><div class="subtitle">Mapping do cliente → camadas → dataset/dataflow → dashboard, com motivo e próxima ação por item.</div></div><div class="badge" id="generated"></div></header>

<section class="grid kpis" id="kpis"></section>

<section class="panel hide" id="fabricPanel"><div class="panel-head"><h2>Presença observada no Fabric DEV</h2><span class="badge" id="fabricAsOf"></span></div><div class="grid plan" id="fabricKpis"></div><p class="note info" id="fabricInterpretation"></p><div class="table-wrap"><table class="matrix"><thead><tr><th>Status estático</th><th>Total</th><th>Local esperado</th><th>Outro domínio/camada</th><th>Não encontrado</th><th>Não aplicável</th></tr></thead><tbody id="fabricMatrix"></tbody></table></div><p class="note warn" id="fabricLimit"></p></section>
...
<div class="field"><label>Status no Fabric</label><select id="fabricStatus"><option value="">Todos</option></select></div>
...
<section class="panel"><div class="panel-head"><h2>Itens do mapping</h2><span class="note">Clique numa linha para inspecionar</span></div><div class="table-wrap"><table><thead><tr><th>Tabela / entrada</th><th>Camada</th><th>Domínio</th><th>Sistema fonte</th><th>Status estático</th><th>Status Fabric</th><th>Local físico</th><th>Notebook candidato</th><th>Dashboards</th></tr></thead><tbody id="rows"></tbody></table></div></section>
<aside class="panel detail"><h2>Diagnóstico</h2><div id="detail" class="empty">Selecione uma linha.</div></aside>
</section>

<section class="panel"><h2>Qualidade do inventário e fontes</h2><div class="grid plan" id="sources"></div><p class="note warn">O status estático descreve a cobertura das relações. O status Fabric descreve presença nominal no snapshot selecionado. Nenhum deles prova execução ou resultado funcional.</p></section>
...
const rows=DATA.rows||[], summary=DATA.summary||{}, fabric=DATA.fabric||{};
...
const fabricClass=s=>s==='NO LOCAL ESPERADO'?'fabric-ok':s==='OUTRO DOMÍNIO/CAMADA'?'fabric-cross':s==='NÃO ENCONTRADO'?'fabric-missing':'noncanonical';
...
if(Object.keys(fabric).length){
 $('eyebrow').textContent='Constellation · Auditoria estática + Fabric DEV';
 $('fabricPanel').classList.remove('hide');
 $('fabricAsOf').textContent='Snapshot '+new Date(fabric.snapshot_collected_at).toLocaleString('pt-BR');
 const fk=[['Tabelas físicas',`${fmt(fabric.physical_found)} / ${fmt(fabric.canonical_tables)}`],['Local esperado',fabric.expected_location],['Outro domínio/camada',fabric.cross_domain_or_layer],['Não encontradas',fabric.not_found],['Origens E2E materializadas',`${fmt(fabric.e2e_origins_materialized)} / ${fmt(fabric.e2e_origins_total)}`],['Notebook candidato',`${fmt(fabric.tables_with_notebook_candidate)} / ${fmt(fabric.canonical_tables)}`],['Reports por nome',fabric.report_names_matched],['Modelos por nome',fabric.semantic_model_names_matched]];
 $('fabricKpis').innerHTML=fk.map(([l,v])=>`<div class="card"><div class="label">${esc(l)}</div><div class="value">${esc(v)}</div></div>`).join('');
 $('fabricInterpretation').textContent='Órfão sem aresta não significa ausente: a tabela pode estar publicada, mas não possuir relação downstream no material analisado. Parcial sem dashboard significa que a cadeia conhecida não alcançou um endpoint de report/dataset.';
 $('fabricMatrix').innerHTML=(fabric.static_fabric_matrix||[]).map(r=>`<tr><td>${esc(statusLabel(r.status))}</td><td>${fmt(r.total)}</td><td>${fmt(r.no_local_esperado)}</td><td>${fmt(r.outro_dominio)}</td><td>${fmt(r.nao_encontrado)}</td><td>${fmt(r.nao_aplicavel)}</td></tr>`).join('');
 $('fabricLimit').textContent=fabric.evidence_limit||'';
}
...
const statuses=[...new Set(rows.map(r=>r.status_fim_a_fim))].sort(), layers=[...new Set(rows.map(r=>r.camada_mapeada).filter(Boolean))].sort(), sources=[...new Set(rows.map(r=>r.sistema_origem).filter(Boolean))].sort(), fabricStatuses=[...new Set(rows.map(r=>r.fabric_status).filter(Boolean))].sort();
...
$('fabricStatus').innerHTML+=""+fabricStatuses.map(x=>`<option>${esc(x)}</option>`).join('');
let selected=null, filtered=[];
function apply(){
 const q=$('search').value.trim().toLocaleLowerCase('pt-BR'), st=$('status').value, la=$('layer').value, so=$('source').value, fs=$('fabricStatus').value, re=$('reaches').value;
 filtered=rows.filter(r=>(!st||r.status_fim_a_fim===st)&&(!la||r.camada_mapeada===la)&&(!so||r.sistema_origem===so)&&(!fs||r.fabric_status===fs)&&(!re||r.chega_dashboard===re)&&(!q||Object.values(r).join(' ').toLocaleLowerCase('pt-BR').includes(q)));
 $('visibleCount').innerHTML=`<span class="count">${fmt(filtered.length)}</span> de ${fmt(rows.length)} itens`;
 $('rows').innerHTML=filtered.map((r,i)=>`<tr data-i="${i}"><td><strong>${esc(r.tabela_mapeada)}</strong></td><td>${esc(r.camada_mapeada)}</td><td>${esc(r.dominio)}</td><td>${esc(r.sistema_origem||'—')}</td><td><span class="status ${statusClass(r.status_fim_a_fim)}">${esc(statusLabel(r.status_fim_a_fim))}</span></td><td><span class="status ${fabricClass(r.fabric_status||'')}">${esc(r.fabric_status||'Não cruzado')}</span></td><td>${esc(r.fabric_localizacoes||'—')}</td><td>${esc(r.fabric_notebook_candidato||'—')}</td><td>${fmt(r.qtd_dashboards)}</td></tr>`).join('')||'<tr><td colspan="9" class="empty">Nenhum resultado.</td></tr>';
...
function select(r,tr){
 ...
 if(r.fabric_status)stages.push({c:'fabric',n:'2. Fabric DEV',v:r.fabric_localizacoes||r.fabric_status,s:r.fabric_status});
 const inner=path.slice(1,-1);inner.forEach(v=>stages.push({c:v.startsWith('DW_')?'silver':(v.startsWith('DM_')||v.startsWith('PR_'))?'gold':'bronze',n:`${stages.length+1}. Dependência`,v,s:'Relação estática'}));
 ...
 const fields=[['Sistema fonte',r.sistema_origem],['Evidência da fonte',r.evidencia_sistema_origem],['Status estático',statusLabel(r.status_fim_a_fim)],['Status no Fabric',r.fabric_status],['Local físico',r.fabric_localizacoes],['Lakehouse ID',r.fabric_lakehouse_ids],['Esperado no Fabric',[r.fabric_lakehouse_esperado,r.fabric_camada_esperada].filter(Boolean).join(' · ')],['Evidência Fabric',r.fabric_evidencia],['Última modificação observada',r.fabric_ultima_modificacao],['Notebook candidato',r.fabric_notebook_candidato],['Notebook ID',r.fabric_notebook_ids],['Pasta do notebook',r.fabric_notebook_pastas],['Report DEV',r.fabric_report_dev],['Report ID atual',r.fabric_report_dev_ids],['Modelo DEV',r.fabric_modelo_dev],['Modelo ID atual',r.fabric_modelo_dev_ids],['ID legado coincide',r.fabric_id_legado_coincide],['Chega a dashboard',r.chega_dashboard],['Etapa alcançada',r.etapa_alcancada],['Confiança',r.confianca],['Tabela endpoint',r.tabela_endpoint],['Qtd. dashboards',r.qtd_dashboards],['Workspace legado',r.workspace],['Dashboard legado',r.dashboard],['Dataset legado',r.dataset],['Caminho conhecido',r.caminho_exemplo],['Dashboards (amostra)',r.dashboards_amostra],['Observação',r.observacao],['Ação recomendada',r.acao_recomendada],['Alertas',r.alertas],['Report ID legado',r.report_id],['Dataset ID legado',r.dataset_id]];
...
['search','status','layer','source','fabricStatus','reaches'].forEach(id=>$(id).addEventListener(id==='search'?'input':'change',apply));
$('clear').onclick=()=>{$('search').value='';$('status').value='';$('layer').value='';$('source').value='';$('fabricStatus').value='';$('reaches').value='';apply()};
